...
export const ProjectSettingsOverrides = Schema.Struct({
  ...
  enableAgentTerminalAccess: Schema.optionalKey(Schema.Boolean),
...
export const ServerSettings = Schema.Struct({
  ...
  /**
   * Whether agents may drive the thread's shared terminals through the
   * `terminal_*` MCP tools, so long-running commands run where the user can
   * watch them. Those terminals run outside the provider's sandbox, so the
   * tools are only attached to full-access sessions even when this is on.
   */
  enableAgentTerminalAccess: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(true))),
  /**
